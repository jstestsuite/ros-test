...
def appendMemory(memory, state, action, reward, next_state):
       return memory.append((state, action, reward, next_state))
...
if __name__ == '__main__':
    ...
    for e in range(EPISODES):
        ...
        env.reset()
	state = np.asarray(env.step([0.0, 0.0] , [0.0,0.0]))
	#print state
	state = state[0:14]
	#reward = 
        score = 0
	    ...
	    loss = 0
	    #print t
            action = actor.model.predict(state.reshape(1, len(state))) + ( actor_noise()*epsilon)
	    
	    
	    #print "ACTION IS PRINTED"
            next_state= env.step((action.flatten()).tolist(), (past_action.flatten()).tolist())
	    reward = next_state[14]
	    next_state = np.asarray(next_state[0:14])
	    #print len(state), "\n"
	    #if (train_indicator): 
          
	    appendMemory(memory, state, action, reward, next_state)
	    #### Start the training
